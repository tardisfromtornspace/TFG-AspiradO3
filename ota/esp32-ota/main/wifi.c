...
void initialise_wifi(const char *running_partition_label)
{
    ...
    {
        ESP_LOGI(TAG, "Previous Wi-Fi credentials from flash memory: %s, %s", wifi_config.sta.ssid, wifi_config.sta.password);
    }
    */
    if (wifi_config.sta.ssid[0] == '\0' || wifi_config.sta.password[0] == '\0')
    {
    ...
    ESP_LOGI(TAG, "Current Wi-Fi credentials stored to flash memory: %s, %s", wifi_config.sta.ssid, wifi_config.sta.password);
